...
exports.getAllProducts = async (req, res) => {
  try {
    const { filter, sortby, order, page = 1, bestSeller, limits = 6 } = req.query
    // mengembalikan total data
    const countData = await productsModels.countAll(filter)

    const productsList = await productsModels.allProducts(filter, sortby, order, page, bestSeller, limits)
