...
allUsers.post("/", uploadMiddlewaree("users").single('picture'), userController.createUsers) // akan melakukan penambahan data baru.
allUsers.patch("/:id", uploadMiddlewaree("users").single('picture'), userController.updateUsers) // akan melakukan update data dengan berdasarkan id yg di kirimkan dari params.
